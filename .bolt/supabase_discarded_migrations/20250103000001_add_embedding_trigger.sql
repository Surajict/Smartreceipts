/*
  # Add Automatic Embedding Generation for Receipts
  
  1. Create function to generate embeddings using Supabase AI
  2. Create trigger to automatically call this function on INSERT/UPDATE
  3. Backfill existing receipts with embeddings
*/

-- Create function to generate embedding for a receipt
...
-- Function to backfill embeddings for existing receipts
  ...
  LOOP
    BEGIN
      ...
      embedding_text := COALESCE(receipt_record.product_description, '') || ' ' ||
                       ...
                       COALESCE(receipt_record.category, '') || ' ' ||
                       COALESCE(receipt_record.subcategory, '');
...
-- Add comment
COMMENT ON FUNCTION generate_receipt_embedding() IS 'Automatically generates vector embeddings for receipts using Supabase AI';
COMMENT ON FUNCTION backfill_receipt_embeddings() IS 'Backfills embeddings for existing receipts without embeddings'; 