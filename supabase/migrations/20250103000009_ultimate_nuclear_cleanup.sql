/*
  # Ultimate Nuclear Cleanup for Embedding System
  
  This migration uses the most thorough approach to clean up ALL embedding-related
  objects including views, functions, triggers, and columns with CASCADE.
*/
...
-- Drop generate functions with ALL possible signatures and CASCADE
...
-- Drop get functions with ALL possible signatures and CASCADE
...
-- Drop search functions with ALL possible signatures and CASCADE
...
-- Drop status functions with ALL possible signatures and CASCADE
...
-- Drop any AI-related functions with CASCADE
...
-- Drop any other potential embedding functions with CASCADE
...
-- Step 2: Drop all indexes that might depend on embedding column
...
DROP INDEX IF EXISTS receipts_embedding_cosine_idx CASCADE;
DROP INDEX IF EXISTS receipts_user_embedding_idx CASCADE;
DROP INDEX IF EXISTS receipts_null_embedding_idx CASCADE;

-- Step 3: Ensure pgvector extension is enabled
CREATE EXTENSION IF NOT EXISTS vector;

-- Step 4: Reset the embedding column completely with CASCADE
DO $$
BEGIN
  -- Drop the embedding column completely with CASCADE if it exists
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'receipts' 
    AND column_name = 'embedding'
  ) THEN
    EXECUTE 'ALTER TABLE receipts DROP COLUMN embedding CASCADE';
    RAISE NOTICE 'Dropped embedding column with CASCADE';
  END IF;
...
-- Step 5: Create the utility function with a unique name to avoid conflicts
...
-- Step 6: Create function to get receipts without embeddings (for backfill)
...
-- Step 8: Create helper function to check embedding status
...
BEGIN
  ...
  RAISE NOTICE '=== ULTIMATE EMBEDDING SYSTEM CLEANUP COMPLETED ===';
  ...
  RAISE NOTICE '';
  RAISE NOTICE 'FUNCTION NAMES (conflict-free):';
  ...
  RAISE NOTICE '';
  RAISE NOTICE 'All dependent objects (views, indexes) dropped with CASCADE';
