/*
  ...
  1. Database Schema Updates
    - Ensure receipts table has all required fields
    - Add proper indexes for performance
    - Update RLS policies for secure access

  2. Storage Configuration
    - Configure receipt-images bucket properly
    - Set up user-specific folder structure
    - Ensure proper file access policies

  3. User Profile Integration
    - Link receipts to authenticated users
...
SELECT 
  ...
  EXTRACT(days FROM calculate_warranty_expiry(r.purchase_date, r.warranty_period) - CURRENT_DATE) as days_until_expiry
FROM receipts r;
