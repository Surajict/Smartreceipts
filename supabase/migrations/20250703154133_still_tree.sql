/*
  # Restore Receipt Scanning Features Migration

  1. Database Schema Updates
    - Ensure all receipt columns exist with proper types
    - Add storage buckets for receipt images and profile pictures
    - Create comprehensive indexes for performance

  2. Security Policies
    - Row Level Security (RLS) for receipts table
    - Storage policies for file uploads
    - User authentication and authorization

  3. Database Functions
    - Receipt validation and processing
    - Warranty calculation and alerts
...
DROP FUNCTION IF EXISTS get_receipts_with_warranty_status(uuid);
DROP FUNCTION IF EXISTS get_user_receipt_stats(uuid);
DROP FUNCTION IF EXISTS search_user_receipts(uuid, text, integer);
DROP FUNCTION IF EXISTS get_user_warranty_alerts(uuid);

-- Function to get receipts with warranty status
CREATE FUNCTION get_receipts_with_warranty_status(user_uuid uuid)
RETURNS TABLE(
  id uuid,
  user_id uuid,
  purchase_date date,
...
CREATE FUNCTION search_user_receipts(
  user_uuid uuid,
  search_query text,
...
CREATE FUNCTION get_user_warranty_alerts(user_uuid uuid)
RETURNS TABLE(
  id uuid,
  product_description text,
  brand_name text,
