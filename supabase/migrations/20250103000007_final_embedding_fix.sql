/*
  # Final Embedding System Fix
  
  This migration completely cleans up and fixes the embedding system by:
  1. Properly removing ALL existing functions with specific signatures
...
-- Step 1: Clean up ALL existing functions and triggers with specific signatures
DROP TRIGGER IF EXISTS trigger_generate_receipt_embedding ON receipts;

-- Drop all variations of the embedding functions with specific signatures
...
-- Drop the problematic get_embedding_status function with all possible signatures
DROP FUNCTION IF EXISTS get_embedding_status();
DROP FUNCTION IF EXISTS get_embedding_status(uuid);
...
-- Step 7: Create helper function to check embedding status (NEW NAME to avoid conflicts)
...
DROP INDEX IF EXISTS receipts_embedding_null_idx;
...
COMMENT ON FUNCTION check_embedding_status IS 'Returns current embedding statistics';
...
BEGIN
  ...
  SELECT * INTO test_status FROM check_embedding_status() LIMIT 1;
  ...
  RAISE NOTICE '1. Deploy the edge functions using: supabase functions deploy';
  RAISE NOTICE '2. Test the functions in your app';
